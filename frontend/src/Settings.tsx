...
export default function Settings() {
  const [savedDevices, setSavedDevices] = useState<DeviceState[]>([]);
  ...
  const loadSavedDevices = async () => {
    try {
      ...
      setError('Failed to load saved devices');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <header className="flex items-center justify-between mb-8">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-4 py-2 rounded-lg font-medium shadow-sm hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
          >
          ...
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden shadow-sm">
            {discovered.length === 0 ? (
              <div className="p-10 text-center text-slate-500 text-sm">
                {scanning ? 'Searching for Gree units...' : 'Click "Scan" to look for new devices on your network.'}
              </div>
            ) : (
              <div className="divide-y divide-slate-100 dark:divide-slate-700">
